...
# =============================================================================
TOTAL_POPS = [10000, 20000, 40000]
GENOME_SIZES = [50, 100]
JUMP_SIZES = [2, 4]
MIGRANT_PCTS = [0.05, 0.10, 0.25, 0.50]  # fraction of island pop
N_THREADS_LIST = [4, 8, 16, 20]
PARALLEL_VARIANTS = [
    "sum",
    "per",
]  # sum: total_pop / threads per island | per: full pop per island

N_RUNS = 5  # repetitions per configuration
TIMEOUT_SEC = 30  # hard kill per binary invocation (seconds)
OUTPUT_DIR = "data"
...
def describe_variant(
    mode, pop, genome, jump, threads, migrants_pct, par_var, island_pop, n_migrants
):
    if mode in ("seq", "rlga"):
        return f"{mode:<10}  pop={island_pop:<6}"
    else:
        return (
            f"{mode}/{par_var:<3}  total_pop={pop:<6}  island_pop={island_pop:<6}  "
...
def main():
    ...
    n_par_combos = (
        len(N_THREADS_LIST) * len(MIGRANT_PCTS) * len(PARALLEL_VARIANTS) * 2
    )  # par + par_rlga
    ...
    for genome, jump, pop in itertools.product(GENOME_SIZES, JUMP_SIZES, TOTAL_POPS):
        output_file = os.path.join(
            OUTPUT_DIR, f"genome{genome}_jump{jump}_pop{pop}.jsonl"
        )
        ...
        for th, pct, var in itertools.product(
            N_THREADS_LIST, MIGRANT_PCTS, PARALLEL_VARIANTS
        ):
            variants.append(("jump_problem", "par", th, pct, var))
            variants.append(("jump_problem_rl", "par_rlga", th, pct, var))

        for v_idx, (binary, mode, threads, migrants_pct, par_var) in enumerate(
            variants, 1
        ):
            ...
            timeout_note = (
                f"  ({timeouts} timeout{'s' if timeouts != 1 else ''})"
                if timeouts
                else ""
            )
            print(
                f"  [{invocation_count}/{total_invocations} done | {pct_done:.1f}%]{timeout_note}"
            )
